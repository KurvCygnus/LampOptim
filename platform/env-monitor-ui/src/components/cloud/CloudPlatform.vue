<template>
  <div class="cloud-platform">
    ...
    <el-row :gutter="20" class="status-row">
      <el-col :span="6">
        <el-card class="stat-card" shadow="hover" :class="{ connected: isConnected }">
          <div class="stat-content">
            <div class="stat-icon" :style="{ background: isConnected ? '#67c23a' : '#909399' }">
              <el-icon size="24" color="#fff"><Connection /></el-icon>
            </div>
            <div class="stat-info">
              <div class="stat-value">{{ isConnected ? "已连接" : "未连接" }}</div>
              <div class="stat-label">{{ getPlatformName(cloudForm.platform) }}</div>
            </div>
          </div>
        </el-card>
      </el-col>
          ...
          >
            ...
            <el-form-item label="数据同步">
              <div class="sync-settings">
                ...
                />
                <span class="sync-text">{{ dataSync ? "已开启" : "已关闭" }}</span>
                <el-select
        ...
        <el-card shadow="hover">
          ...
          <el-table :data="deviceMappings" style="width: 100%">
            ...
            <el-table-column label="同步状态" width="100">
              <template #default="scope">
                <el-tag :type="scope.row.syncEnabled ? 'success' : 'info'" size="small">
            ...
            <el-table-column label="操作" fixed="right" width="200">
              <template #default="scope">
                <el-button type="primary" size="small" @click="syncSingleDevice(scope.row)">
                  同步
                </el-button>
                <el-button type="warning" size="small" @click="editMapping(scope.row)">
                  编辑
                </el-button>
                <el-button type="danger" size="small" @click="deleteMapping(scope.row)">
                  删除
                </el-button>
              </template>
            </el-table-column>
        ...
        <el-card shadow="hover">
          ...
          <el-table :data="alertRules" style="width: 100%">
            ...
            <el-table-column label="操作" fixed="right" width="150">
              <template #default="scope">
                <el-button type="primary" size="small" @click="editAlertRule(scope.row)">
                  编辑
                </el-button>
                <el-button type="danger" size="small" @click="deleteAlertRule(scope.row)">
                  删除
                </el-button>
              </template>
            </el-table-column>
        ...
        <el-card shadow="hover">
          <template #header>
            <div class="card-header">
              <span>云端实时数据预览</span>
              <div class="header-actions">
                <el-button type="primary" size="small" @click="refreshRealtimeData" :loading="refreshing">
                  <el-icon><Refresh /></el-icon>
                  刷新数据
                </el-button>
                <el-button type="success" size="small" @click="exportRealtimeData">
                  <el-icon><Download /></el-icon>
                  导出数据
                </el-button>
              </div>
    ...
    <!-- 设备映射对话框 -->
    <el-dialog v-model="mappingDialogVisible" :title="isEditMapping ? '编辑映射' : '添加映射'" width="500px">
      <el-form :model="mappingForm" label-width="100px">
        <el-form-item label="本地设备">
          <el-select v-model="mappingForm.localDevice" placeholder="选择本地设备" style="width: 100%">
            <el-option label="温湿度传感器-01" value="temp_hum_01" />
            <el-option label="光照传感器-01" value="light_01" />
            <el-option label="空气质量传感器-01" value="air_01" />
            <el-option label="路灯控制器-01" value="lamp_01" />
          </el-select>
        </el-form-item>
        <el-form-item label="云端产品">
          <el-input v-model="mappingForm.cloudProduct" placeholder="输入云端产品名称" />
        </el-form-item>
        <el-form-item label="云端设备">
          <el-input v-model="mappingForm.cloudDevice" placeholder="输入云端设备名称" />
        </el-form-item>
    ...
    <!-- 告警规则对话框 -->
    <el-dialog v-model="alertDialogVisible" :title="isEditAlert ? '编辑告警规则' : '添加告警规则'" width="550px">
      <el-form :model="alertForm" label-width="100px">
        <el-form-item label="规则名称">
          <el-input v-model="alertForm.name" placeholder="输入规则名称" />
        </el-form-item>
        <el-form-item label="关联设备">
          <el-select v-model="alertForm.device" placeholder="选择设备" style="width: 100%">
        ...
        <el-form-item label="监控指标">
          <el-select v-model="alertForm.metric" placeholder="选择指标" style="width: 100%">
...
const rules = {
  platform: [{ required: true, message: "请选择云平台类型", trigger: "change" }],
...
const deviceMappings = ref([
  { id: 1, localDevice: "temp_hum_01", localType: "传感器", cloudDevice: "device_001", cloudProduct: "环境监测", syncEnabled: true, lastSync: "2024-01-15 10:30:00" },
  { id: 2, localDevice: "light_01", localType: "传感器", cloudDevice: "device_002", cloudProduct: "环境监测", syncEnabled: true, lastSync: "2024-01-15 10:30:00" },
  { id: 3, localDevice: "lamp_01", localType: "控制器", cloudDevice: "device_003", cloudProduct: "智能路灯", syncEnabled: false, lastSync: "--" },
]);

const syncLogs = ref([
  { id: 1, time: "2024-01-15 10:30:00", success: true, type: "自动同步", device: "temp_hum_01", dataCount: 120, duration: 156, message: "同步成功" },
  { id: 2, time: "2024-01-15 10:25:00", success: true, type: "自动同步", device: "light_01", dataCount: 85, duration: 98, message: "同步成功" },
  { id: 3, time: "2024-01-15 10:20:00", success: false, type: "手动同步", device: "lamp_01", dataCount: 0, duration: 0, message: "连接超时" },
  { id: 4, time: "2024-01-15 10:15:00", success: true, type: "自动同步", device: "air_01", dataCount: 200, duration: 234, message: "同步成功" },
]);
...
const alertRules = ref([
  { id: 1, name: "高温告警", device: "all", metric: "温度", operator: ">", threshold: 35, unit: "°C", notifyMethods: ["短信", "邮件"], enabled: true },
  { id: 2, name: "设备离线告警", device: "all", metric: "设备离线", operator: ">", threshold: 5, unit: "分钟", notifyMethods: ["短信", "钉钉"], enabled: true },
  { id: 3, name: "PM2.5超标", device: "air_01", metric: "PM2.5", operator: ">", threshold: 75, unit: "μg/m³", notifyMethods: ["邮件"], enabled: false },
]);
...
const getLogTypeTag = (type) => {
  const tags = { "自动同步": "primary", "手动同步": "warning", "定时同步": "success" };
...
const connectCloud = async () => {
  if (isConnected.value) {
    try {
      await ElMessageBox.confirm("确定要断开云平台连接吗？", "提示", { type: "warning" });
...
const resetConfig = () => {
  Object.assign(cloudForm, { platform: "aliyun", productKey: "", deviceName: "", accessKey: "", secretKey: "", region: "cn-shanghai" });
...
const updateSyncInterval = () => {
  if (dataSync.value) { stopSync(); startSync(); }
};
...
const syncData = async () => {
  ...
  } catch { syncFailedCount.value++; }
};

const addMapping = () => {
  isEditMapping.value = false;
  Object.assign(mappingForm, { id: null, localDevice: "", cloudProduct: "", cloudDevice: "", syncEnabled: true });
...
const saveMapping = () => {
  if (isEditMapping.value) {
    const index = deviceMappings.value.findIndex((m) => m.id === mappingForm.id);
    if (index > -1) Object.assign(deviceMappings.value[index], mappingForm);
  } else {
    deviceMappings.value.push({ ...mappingForm, id: Date.now(), localType: "传感器", lastSync: "--" });
  }
...
const deleteMapping = (row) => {
  ElMessageBox.confirm(`确定删除设备 ${row.localDevice} 的映射吗？`, "提示", { type: "warning" })
    .then(() => {
      deviceMappings.value = deviceMappings.value.filter((m) => m.id !== row.id);
...
const clearLogs = () => {
  ElMessageBox.confirm("确定要清空所有同步日志吗？", "警告", { type: "warning" })
    .then(() => {
      syncLogs.value = [];
...
const addAlertRule = () => {
  ...
  Object.assign(alertForm, { id: null, name: "", device: "", metric: "", operator: ">", threshold: 0, unit: "", notifyMethods: [], enabled: true });
...
const deleteAlertRule = (row) => {
  ElMessageBox.confirm(`确定删除告警规则 ${row.name} 吗？`, "提示", { type: "warning" })
    .then(() => {
...
const updateChart = () => {
  ...
  chart.setOption({
    ...
    series: [
      { name: "温度", type: "line", smooth: true, data: tempData, itemStyle: { color: "#f56c6c" }, areaStyle: { color: "rgba(245, 108, 108, 0.1)" } },
      { name: "湿度", type: "line", smooth: true, yAxisIndex: 1, data: humData, itemStyle: { color: "#409eff" }, areaStyle: { color: "rgba(64, 158, 255, 0.1)" } },
    ],
  });
};
...
const initConfig = () => {
  ...
  if (savedConfig) {
    try { Object.assign(cloudForm, JSON.parse(savedConfig)); } catch {}
  }
};

onMounted(() => {
  initConfig();
  nextTick(() => { if (activeTab.value === "realtime") initChart(); });
});
